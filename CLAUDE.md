...
A4CPL (AI-Assisted Assessment to Award Credit for Prior Learning) is a Next.js 14 application that uses AI (OpenAI) to conduct adaptive, conversational assessments for awarding college credit based on prior experience. The initial domain is Fire Technology courses (e.g., FIRETEK 217 - Wildland Fire Control).
...
- `prompts/` — Externalized prompt templates (JSON files editable without code changes)
...
The AI's role is to direct conversation only — it does not evaluate or score student performance. The core function is:
- `generateAssessmentQuestion()` — Sends conversation history to OpenAI with a system prompt built from course context and externalized prompt templates (`prompts/assessment.json`); produces scenario-based, follow-up, or clarification questions
...
### Prompt Organization
System prompts are stored in `prompts/assessment.json` and loaded at runtime. This allows prompt text to be edited without modifying code. See `_documentation` field in the JSON file for details on each prompt's purpose and available placeholders.
...
- `OPENAI_API_KEY` — Required for AI assessment features
